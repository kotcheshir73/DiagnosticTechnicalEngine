...
namespace ServicesModule
{
    public class ModelDiagnosticTest
    {
        ...
        public List<double> GetForecastByPointTrend(int id)
        {
            using (var _context = new DissertationDbContext())
            {   // 0 - находиим диагностический тест
                var diagTest = _context.DiagnosticTests
                ...
                var xNow = Converter.ToFuzzyTrendLabelWeight(LastPoint.FuzzyTrend.TrendName);
                if (xNow == Converter.TrendWeightNotFound)
                {
                    throw new Exception(string.Format("Не найден вес для тенденции {0}", LastPoint.FuzzyTrend.TrendName));
                }
                var xLast = Converter.ToFuzzyTrendLabelWeight(PreLastPoint.FuzzyTrend.TrendName);
                if (xLast == Converter.TrendWeightNotFound)
                {
                    throw new Exception(string.Format("Не найден вес для тенденции {0}", PreLastPoint.FuzzyTrend.TrendName));
                }
                // определяем точку фазовой плоскости для последней точки ряда
                int pointTrend = ModelCalculator.CalcPointOnPhasePlane(LastPoint.FuzzyTrend.TrendName, xLast - xNow);

                var futurePoints = _context.PointTrends.Where(pt => pt.StartPoint == pointTrend && pt.SeriesDiscriptionId == diagTest.SeriesDiscriptionId);
                List<double> results = new List<double>();
                foreach (var fPount in futurePoints)
                {
                    results.AddRange(CalcByPointTrend(fPount.Trends, LastPoint.Value.Value, PreLastPoint.Value.Value));
                }
                return results.Distinct().ToList();
            }
        }

        private List<double> CalcByPointTrend(string trendNames, double value, double preValue)
        {
            List<double> results = new List<double>();
            var trendNamesSplit = trendNames.Split(',');
            foreach (var trendName in trendNamesSplit)
            {
                FuzzyTrendLabel label = Converter.ToFuzzyTrendLabel(trendName);
                switch (label)
                {
                    case FuzzyTrendLabel.СтабильностьСредняя:
                        results.Add(value);
