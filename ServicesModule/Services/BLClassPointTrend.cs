...
{
    public class BLClassPointTrend
	{
		...
		public IEnumerable<PointTrendViewModel> GetListPointTrend(int parentId)
		{
			try
			{
				using (var _context = new DissertationDbContext())
				{
					...
					.Where(rec => rec.SeriesDiscriptionId == parentId).ToList()
					.OrderBy(rec => rec.StartPoint)
					.Select(rec => ModelConvector.ToPointTrend(rec));
				}
			}
			catch (Exception ex)
			{
		...
		public PointTrendViewModel GetElemPointTrend(int id)
		{
			try
			{
				using (var _context = new DissertationDbContext())
				{
					return ModelConvector.ToPointTrend(_context.PointTrends.SingleOrDefault(rec => rec.Id == id));
				}
			}
			catch (Exception ex)
			{
		...
		public bool AddPointTrend(PointTrendBindingModel model)
		{
			using (var _context = new DissertationDbContext())
			{
				try
				{
					var list = _context.PointTrends.Where(rec => rec.SeriesDiscriptionId == model.SeriesId);
					if (list.FirstOrDefault(rec => rec.StartPoint == model.StartPoint &&
													rec.FinishPoint == model.FinishPoint) != null)
					{
		...
		public bool UpdPointTrend(PointTrendBindingModel model)
		{
			using (var _context = new DissertationDbContext())
			{
				...
				{
					var list = _context.PointTrends.Where(rec => rec.SeriesDiscriptionId == model.SeriesId);
					var elem = _context.PointTrends.SingleOrDefault(rec => rec.Id == model.Id);
					if (list.FirstOrDefault(rec => rec.Id != model.Id &&
													rec.StartPoint == model.StartPoint &&
													rec.FinishPoint == model.FinishPoint) != null)
					{
						_error = "Уже есть такое правило!";
						return false;
		...
		public bool DelPointTrend(int id)
		{
			using (var _context = new DissertationDbContext())
			{
				_context.PointTrends.Remove(_context.PointTrends.Single(rec => rec.Id == id));
				_context.SaveChanges();
				return true;
			}
		}
		...
		public bool DelPointTrendFromSeries(int seriesId)
		{
			using (var _context = new DissertationDbContext())
			{
				_context.PointTrends.RemoveRange(_context.PointTrends.Where(rec => rec.SeriesDiscriptionId == seriesId));
				_context.SaveChanges();
				return true;
			}
		}
		...
		public bool CalcPointsTrend(PointTrendCalcBindingModel model)
		{
			...
			var FuzzyLabel = new BLClassFuzzyLabel();
			...
			var rulesTrend = new BLClassRuleTrends();
