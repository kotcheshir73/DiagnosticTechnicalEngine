    ...
    public class DiagnosticTestService
    {
            ...
            using (var _context = new DissertationDbContext())
            {
                try
                {
                    ...
                    {//если уже есть точки, получить тенденцию
                        ...
                        var rule = _context.RuleTrends.SingleOrDefault(r => r.FuzzyLabelFromId == labelFromId && r.FuzzyLabelToId == point.FuzzyLabelId &&
            ...
            using (var _context = new DissertationDbContext())
            {
                try
                {
                    ...
                    var stateEntropy = _context.StatisticsByEntropys.SingleOrDefault(r =>
                        ...
                        r.DiagnosticTestId == point.DiagnosticTestId);
                    if (stateEntropy == null)
                    {
                        var number = _context.StatisticsByEntropys.Where(sbf => sbf.DiagnosticTestId == point.DiagnosticTestId).Select(sbf => sbf.NumberSituation).DefaultIfEmpty().Max() + 1;
        ...
        private StatisticsByFuzzy GetStateFuzzy(PointInfo point)
        {
            using (var _context = new DissertationDbContext())
            {
                try
                {
                    ...
                    if (stateFuzzy == null)
                    {
                        var number = _context.StatisticsByFuzzys.Where(sbf => sbf.DiagnosticTestId == point.DiagnosticTestId).Select(sbf => sbf.NumberSituation).DefaultIfEmpty().Max() + 1;
        ...
        private void AnomalyDetectedByElem(StatisticBy statistic, TypeSituation type)
        {
            using (var _context = new DissertationDbContext())
            {
                ...
                var listAnomaly = _context.AnomalyInfos.Where(ai => ai.TypeSituation == type && ai.DiagnosticTestId == statistic.DiagnosticTestId).ToList();
        ...
        private void CheckNewStateByElem(StatisticBy statistic, TypeSituation type, PointInfo point)
        {
            using (var _context = new DissertationDbContext())
            {
                ...
                {//новая аномалия
                    ...
                    {
                        _evMessage?.Invoke("Точка №" + _countAddedPoints + ". Зафиксирована аномалия по энтропии, но имеется менее 5 точек для ее идентифкации: " + setSituations);
                    }
                    else
                ...
                try
                {
                    ...
                    {//
                        var stateEntropy = _context.StatisticsByEntropys.SingleOrDefault(r => r.NumberSituation == anomaly.AnomalySituation &&
                                                    r.DiagnosticTestId == anomaly.DiagnosticTestId);
                        if (stateEntropy == null)
                        {
                            if (_evMessage != null)
                            {
